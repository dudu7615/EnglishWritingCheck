...
def run(port: int, timeout: int = 10) -> str | None:
    ...
    stdoutThread = threading.Thread(target=_read_output, args=(process.stdout, queue), daemon=True)
    stderrThread = threading.Thread(target=_read_output, args=(process.stderr, queue), daemon=True)
    stdoutThread.start()
    stderrThread.start()
